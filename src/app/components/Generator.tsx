...
function ratioWCAG(bg: string, fg: string) {
  ...
  try {
    const b = toRGB(hex(bg.padEnd(7,'0')))
    const f = toRGB(hex(fg.padEnd(7,'0')))
...
export function Generator({ initialValues }: { initialValues: Partial<FormValues> }) {
  ...
  const { register, formState: { errors }, watch, getValues } = useForm<FormValues>({
  ...
  const baseUrl = useMemo(() => (typeof window === 'undefined' ? 'http://localhost:3000' : window.location.origin), [])
  ...
  return lines.join('\n') // << AQUI: precisa do \n, não quebra de linha literal
}, [values, ogURL])

  const contrast = useMemo(() => ratioWCAG(values.ogBg, values.ogFg), [values.ogBg, values.ogFg])
  const contrastMsg = contrast >= 4.5 ? 'Bom contraste (≥ 4.5:1)' : contrast >= 3 ? 'Contraste ok para texto grande (≥ 3:1)' : 'Contraste baixo — ajuste as cores'
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1.1fr,0.9fr] gap-8">
      ...
      <section aria-labelledby={`${id}-form-title`} className="space-y-6">
        <header className="space-y-1">
          ...
          <p id={`${id}-form-help`} className="text-sm opacity-80">Preencha os campos. Tudo reflete no preview e no snippet abaixo. Campos marcados como obrigatórios precisam de valor.</p>
        </header>
        ...
        <form aria-describedby={`${id}-form-help`} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input id={`${id}-title`} aria-invalid={!!errors.title} aria-errormessage={errors.title ? `${id}-title-err` : undefined} className="w-full rounded-xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-zinc-900/60 px-3 py-2 outline-none focus-visible:ring-2 focus-visible:ring-sky-500" {...register('title')} />
          ...
          <div>
            ...
            <input id={`${id}-site`} className="w-full rounded-xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-zinc-900/60 px-3 py-2" {...register('siteName')} />
          </div>

          {/* Descrição */}
          <div className="sm:col-span-2">
            <label htmlFor={`${id}-desc`} className="text-sm">Descrição *</label>
            <textarea id={`${id}-desc`} rows={3} aria-invalid={!!errors.description} aria-errormessage={errors.description ? `${id}-desc-err` : undefined} className="w-full rounded-xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-zinc-900/60 px-3 py-2" {...register('description')} />
          ...
          <div className="sm:col-span-2">
            ...
            <input id={`${id}-canon`} aria-invalid={!!errors.canonical} aria-errormessage={errors.canonical ? `${id}-canon-err` : undefined} className="w-full rounded-xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-zinc-900/60 px-3 py-2" {...register('canonical')} />
          ...
          <div>
            ...
            <select id={`${id}-type`} className="w-full rounded-xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-zinc-900/60 px-3 py-2" {...register('type')}>
          ...
          <div>
            ...
            <select id={`${id}-tw`} className="w-full rounded-xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-zinc-900/60 px-3 py-2" {...register('twitterCard')}>
          ...
          <div>
            ...
            <input id={`${id}-author`} className="w-full rounded-xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-zinc-900/60 px-3 py-2" {...register('author')} />
          </div>
          ...
          <fieldset className="sm:col-span-2 border border-black/10 dark:border-white/10 rounded-2xl p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                ...
                <input id={`${id}-ogtext`} className="w-full rounded-xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-zinc-900/60 px-3 py-2" {...register('ogImageText')} />
              </div>
              <div>
                <label htmlFor={`${id}-ogbg`} className="text-sm">Cor de fundo</label>
                <div className="flex gap-2 items-center">
                  <input id={`${id}-ogbg`} type="color" className="h-10 w-14 rounded-md border border-black/10 dark:border-white/10" {...register('ogBg')} />
                  <input aria-label="Hex da cor de fundo" className="flex-1 rounded-xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-zinc-900/60 px-3 py-2 font-mono" {...register('ogBg')} />
                </div>
              </div>
              <div>
                <label htmlFor={`${id}-ogfg`} className="text-sm">Cor do texto</label>
                <div className="flex gap-2 items-center">
                  <input id={`${id}-ogfg`} type="color" className="h-10 w-14 rounded-md border border-black/10 dark:border-white/10" {...register('ogFg')} />
                  <input aria-label="Hex da cor do texto" className="flex-1 rounded-xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-zinc-900/60 px-3 py-2 font-mono" {...register('ogFg')} />
                </div>
      ...
      {/* Preview + Snippet com acessibilidade */}
      <section aria-labelledby={`${id}-preview-title`} className="space-y-6">
        ...
        <div className="rounded-2xl border border-black/10 dark:border-white/10 overflow-hidden shadow-sm">
          <div className="p-3 text-sm opacity-80 border-b border-black/5 dark:border-white/5 flex items-center justify-between">
            ...
            <button type="button" onClick={copySnippet} className="text-xs underline opacity-80 hover:opacity-100 focus-visible:ring-2 focus-visible:ring-sky-500 rounded-md px-1" aria-live="polite" aria-label="Copiar snippet para a área de transferência">
